...
- `ApplicationJob`, `R3x::RunWorkflowJob`, `R3x::Workflow::Base`, and `R3x::ChangeDetectionJob` add stable tagged log context such as `r3x.run_active_job_id`, `r3x.workflow_key`, and `r3x.trigger_key` so indexed logs can be correlated back to workflow and run pages. The workflow job itself keeps the per-run tags minimal (`r3x.run_active_job_id` and `r3x.trigger_key`), while orchestration jobs still emit `r3x.workflow_key` for broader workflow-level correlation.
