...
- When `R3X_LOGS_PROVIDER=victorialogs`, prefer structured JSON logger output with explicit `level`, `message`, and tag data so the dashboard can read real log levels directly. Without a logs provider, prefer the flatter local text formatter for readability.
