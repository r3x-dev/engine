...
- `app/models/dashboard/`: dashboard-facing models over persisted queue metadata, especially `Dashboard::Run` (`solid_queue_jobs` + execution-table relations + direct enqueue) and `Dashboard::RecurringTask` (`solid_queue_recurring_tasks` parsing and workflow-key lookups).
