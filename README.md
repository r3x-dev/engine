...
- `bin/jobs` remains the generic Solid Queue entrypoint. For split-process deployments, prefer `bin/jobs-worker` and `bin/jobs-scheduler` so the pod role is encoded in the command rather than a separate env var. They still honor explicit `SOLID_QUEUE_CONFIG` overrides when needed.
