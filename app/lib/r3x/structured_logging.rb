module R3x
  module StructuredLogging
    def structured_error(message:, error:)
      if R3x::Log.json?
        payload = { level: "error", time: Time.current.utc.iso8601(6), message: message, error_class: error.class.name, error_message: error.message, backtrace: error.backtrace&.first(20), tags: current_log_tags }
